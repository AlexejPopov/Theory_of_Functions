...
раммы. Функции создается имя, и по этому имени идет связь с функцией. Функции создаются для фрагментов кода повторяющихся в программе в разных местах несколько раз,
которые можно вынести за тело программы (за ф.main). Этот фрагмент кода получает собственное имя и далее может быть использован в нужном месте при указании его имени.
...
называется объявление функции, а тело - реализация функции.
                                                                       ...
                                                                       Заголовок состоит из 3-х пунктов:
1. Имя функции (задается нами при объявлении функции)
2. Параметры (аргументы) функции - значения, которые нужно в функцию передать, чтобы она смогла на их основании выполнить какие-то действия. Это входные данные, необ-
   ходимые функции для работы над кодом. В качестве параметров можно использовать много чего, но чаще всего переменные.
...
Если у функции нет одиночного однозначного значения или оно неизвестно, напр. функция лишь что-то выводит на экран консоли, то она при этом никаких вычислений, мат.о-
пераций и прочего не выполняет, след, и осознанного результата у нее тоже нет. Для нее должен быть указан тип возвращаемого значения void. В данном случае просто ис-
пользуется содержимое функции в виде вывода на экран ее данных и все!
...
II. Объявление функции прототипом с реализацией после ф.main
...
лишь текст на экран, то ей никаких параметров не надо (текст везде одинаковый, без изменений), () будут пустыми. Затем, ставим блоковые фиг.скобки, в которых находит-
ся тело функции.

В тех местах, где нужно использовать содержимое новой функции (напр. вывести текст на экран), нужно лишь обратиться к ней (вызвать функцию). Чтобы вызвать функцию надо
написать ее имя и передать ей параметры, необходимые для ее работы. Но т.к. моей функции Hello никаких параметров для работы не требуется, то вызовется она тоже без
параметров. Достаточно указать пустые () скобки. Но () скобки обязательны, и даже без параметров, т.к. именно () скобки обозначают вызов функции. В конце обязательна
';' т.к. вызов функции это законченная мысль. */
...
личество звездочек для вывода будет задано, функция не будет ничего вычислять, а след, и возвращать результат она не будет, введу void. А принимать она будет количест-
во звездочек в качестве параметра, след, в круглых скобках надо указать параметр, его тип данных и имя. У количества звездочек будет int. Функция содержит переменную,
которую надо задать для выполнения цикла, но в самой функции cin нет, след, нужно задать значение переменной при вызове функции. Это называется принять 1 параметр (1
переменную). Принятие осуществляется заданием переменной в круглых скобках при вызове функции.

    Как работают функции?
В момент вызова функции основная ф.main встает на паузу. Она стопорится и управление переходит в мою функцию (туда где функция инициализировалась/лежит в ОЗУ), функ-
ция начинает работу. До тех пор пока она работает ф.main стоит на паузе. Как только все действия, указанные в теле функции выполнятся, компилятор вернется в место вы-
зова функции и продолжит дальнейшие действия по коду программы. Это ярко видно в отладчике (Step Into/Шаг с заходом). Это отладка с заходом в функции, которые написаны
для моей программы. Передам значение '5' и распишу механику:
...
получат значения, переданные при вызове функции (например 5).
(!) все, что создается внутри функции, доступно только внутри функции и пока функция работает.
Как только функция закончила работу, п.count исчезнет и больше к ней доступа не будет. А след, имена параметров функции могут быть точно такими же как и названия дру-
гих объектов кода. Они никак не будут связаны. count в ф.main и count в ф.Stars это абсолютно разные переменные, никак не связанные между собой. count в Stars живет
только в ф.Stars, а count из main живет только в ф.main. Когда я вызову ф.Stars и напишу в ней count из main, то ЗНАЧЕНИЕ п.count из main перейдет в count из Stars и
ф.Stars будет с этим значением работать. */
...
/* Т.о. передается не сама переменная в функцию, а лишь ее значение, след, в ф.main передающая переменная может иметь любое имя, необязательно такое же как в ф.Stars.
Также можно перед передачей значения в функцию, ввести это значение с клавиатуры. 
Пусть ф.Print_line будет выводить на экран любой символ, след, она будет принимать 2 параметра (количество символов и сам символ). Функция будет выводить на экран ли-
нию из символов и не будет ничего возвращать, ни площадь, ни рассчет з/п, она просто будет выводить звездочки на экран. */
...
/* Сигнатура функции это весь заголовок функции. В нее входят возвращаемое значение (напр. void/int), имя функции и параметры (их количество и типы данных).
...
передавать в нее параметры иных типов данных, иначе случится неявное (расширяющее/сужающее) преобразование типов данных с возможной потерей данных. Отсюда 2 правила:

    - ТИПЫ ДАННЫХ ПРИ ВЫЗОВЕ ФУНКЦИИ И В ЕЕ ЗАГОЛОВКЕ ДОЛЖНЫ СОВПАДАТЬ!
    - ПОРЯДОК ТИПОВ ДАННЫХ НЕ ДОЛЖЕН НАРУШАТЬСЯ!

Если первый идет char, а потом int, то только так необходимо вызывать функцию (char, а потом int).
(!) void Print_line(char symbol, int count); и
...
Пусть ф.MyPow, возвращает результат своей работы. Она будет возводить в степень целое цисло. Результатом функции будет число, возведенное в степень, след, нужно ука-
зать тип данных этого результата. Если целое число возведу в целую степень, то получу целый результат (int). Далее, чтобы функция смогла вернуть результат, нужна пере-
менная, в которую результат поместится. int result будет хранить промежуточный и конечный результат умножения. Далее в цикле for перемножатся п.digit и result. Чтобы вернуть
