...
(!) переменные созданные внутри функций создаются до запуска ф.main, т.к. функции это глоб.объекты, словно глоб.п./к., создаются в ОЗУ до вызова ф.main.
...
int main()
{
    ...
    Swap(A, B);
    cout << "Число А: " << A << " Число В: " << B << endl;         // люб. функция работает не с переменными, а с их значениями. Сами переменные остаются без изменений.

    /* С массивами же немного иначе.
    Напишу функцию, вычисляющую количество четных элементов массива. Функция будет возвращать количество четных элементов.
    Массив характеризуется типом данных своих элементов и их количеством. След, передавая массив в функцию необходимо отдельно передать его ИМЯ, указав тип данных и ко-
    личество элементов, которые в массиве хранятся. Иначе при работе с массивом можно выйти за границы массива.
