...
* Вызову функцию, передав ей лишь 2 параметра [2]. Вводя 2 параметра, заполнены будут 1-ые два параметра, а последний получит значение по умолчанию. Только так. Нельзя
дать значение 1-му и 3-му значению, а 2-му придать значение по умолчанию, напр, при помощи запятых с пробелом, т.к. параметры функции в памяти располагаются непрерывно,
один за другим. Задавать значения надо с конца и не указывать их тоже с последнего. Т.о. в symbol введу (*), в rows (3), а количество столбцов будет взято из параметров
по умолчанию (10). Итого будет 3 строки по 10 звездочек.
